...
- STL
- Шаблоны
Что это значит? Что у нас в разных областях применяются разные правила, и то что хорошо делать в одном месте, плохо делать в другом. До сих пор мы затрагивали только первый раздел. Сегодня рассмотрим STL и некоторые принципы которые пришли вместе c С++.
STL - это супер мега крутая библиотека (стандартная), которую ты уже использовал на самом деле, везде где ты писал std::что-то это STL!
