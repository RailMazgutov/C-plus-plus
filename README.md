...
- [Урок 6](https://github.com/RailMazgutov/C-plus-plus#Урок-6)
  ...
  - [Наследование](https://github.com/RailMazgutov/C-plus-plus#Наследование)
  - [Задание 6](https://github.com/RailMazgutov/C-plus-plus#Задание-6)
...
# Урок 5
### Коротко о том что сейчас начнется. 
Методы играют большую роль в ООП, они определяют поведение объектов. По такой фразе не понятно что это такое. В структуре есть переменные (которые называются полями), они определяют состояние объекта. Т.е. изменяе поля объекта мы изменяем его состояние. Способность переходить из одного состояния в другое называется поведением. В самых простых случаях изменить состояние системы можно напрямую, изменив значения переменных. Иногда это не так просто, и для этого создаются специальные функции, если эти функции объявлены внутри структуры, они называются методами.
...
## Методы
Вспомним как мы объявляем структуру:
```
// Определяем тип комплесное число
struct Complex // Чаще всего имя пользовательского типа надо начинать с заглавной буквы
{
    // Поля класса
    double real;
    double img;
...
```
Ты уже реализовывал функции для сложения двух чисел, сделаем из них методы:
```
struct Complex
{
    // Поля класса
    ...
    double img;
    // Методы класса
    Complex add(const Complex& complex); // Объявление метода
};
```
Легко заметить, что объявление метода, который функция внутри типа, на самом деле отличается от объявления соответствующей функции (которую ты реализовывал ранее):
```
Complex sum(const Complex& number1, const Complex& number2);
```
Ну кроме того что она называется по другому, она принимает ссылку только на один объект, где взять второй?

Обо всём по порядку. Она называется по другому `add - прибавить`, когда `sum - сумма`. Т.е. раньше мы находили сумму двух чисел, а теперь к одному числу рибавляем другое. 

Давай посмотрим как определить метод:
```
// Сначла указываем тип возвращаемых данных
// Потом полное имя функции
// В скобках перечисляем аргументы
Complex Complex::add(const Complex& complex)
{
  Complex result; // Объявляем объект, который вернем
  result.real = real + complex.real;
  result.img = img + complex.img;
  return result; // Возвращаем объект
}
```
Тут две особенности на которые стоит обратить внимание.
- Полное имя функции `Complex::add`, имя структуры два двоеточия (`::`) имя метода. Это всегда так, просто нужно запомнить.
- При вычилении значений полей нового объекта: `result.real = real + complex.real;`, что это за `real`, откуда он взялся и чей он? Ответ прост, это значение поля оъекта, к которому применили метод.

Кстати о применении метода, рассмотрим как это делается:
```
  Complex complex1 {0, 1};
  Complex complex2 {1, 0};
  Complex complex3 = complex1.add(complex2);
  output(complex3); // Это практически та же самая функция, которую реализоввал ты.
```
Наверное интересно что мы получили на выходе?
```
1+i1
```
В этом примере мы применяем метод к объекту `complex1`, значит внутри метода будут напрямую (без указания объекта) доступны поля этого объекта. Тут всплывает интересное и важное свойство методов:

**Методы имеют непосредственный доступ ко всем полям и методам класса.**

Ты должен сказать "АГА! ко всем полям и _методам_!" и будешь молодец. Да и к методам, внутри одного метода можно вызвать другой метода без указания объкта, к которому он применяется. Может показаться запутанным, поэтому приведу пример. Давай сделаем функцию output методом:
```
struct Complex
{
    // Поля класса
    ...
    void output() const; // Новый метод, как видишь он ничего не принимает на вход
};
```
...
result: 1+i1
```
Все шикарно, и все круто. Но должен появиться еще один вопрос: **"Что блин за `const` в определении и объявлении метода?"** Если такой вопрос появился, молодец - 5 за внимательность! Этот `const` необходим, для того что бы сказать, что наш метод не изменяет состояние объекта. Т.е. не изменяет его полей. И Если сейчас появилось негодование: "А метода `add` то же не изменяет состояние, где его `const`?" Вторая 5 за внимательность. Да он там нужен! Итак код всей программы:
```
#include <iostream>
using namespace std;
...
```
### Самое важное:
- Методы это функции объявленные внутри класса или структуры.
- Полное имя метода (как функции) _ИмяКласса::имяМетода_.
- Определение метода происходит вне объвления класса.
...
## Строки
Строки это важно, строки это круто. Например, нужно написать программу, которая спрашивает у пользователя как его зовут, а потом подставляет его имя в нужные места в документе. Или еще что делает с его именем.
...
```
Все просто, объявляем массив, записываем в него введенное значение потом выдаем. Как это работает? Строки в стиле "Си" хранят на 1 символ больше, чем мы видим. Они заканчиваются на символом '\0', который называется символом конца строки. Подробнее о них можно почитать [тут](http://cppstudio.com/post/437/).

Но сейчас это НЕ круто! Круто делать так:
```
#include <iostream>
using namespace std;
...
```
Очень удобно, и есть куча встроенных методов. Да это на самом деле класс, и у него куча встроенных методов. А еще не надо париться влезет ли введённая строка в массив (он автоматически расширяется (ничего не напоминает?)). Подробнее обо всех методах можно прочитать [тут](http://ru.cppreference.com/w/cpp/string/basic_string).
## Задание 5
1. Реализовать операторы +, -, * , / для комплексных чисел. (так же предусмотреть действия с действительными числами)

2. Пользователь вводит строку, вида `"+++--++-+-+---+"` заменить в этой строке все + на -, а все - на +.

3. Создать структуру, которая представляет студента. Его имя, фамилию, и его оценка по 100 бальной шкале. Пользователь вводит число студентов в группе. Потом вводит данные фамилию, имя, оценка. (Запрашиваешь ввести) Потом отсортировать список на основе оценок и вывести список. После вывести среднею оценку.

4. Реализовать структуру хранения данных список строк. В качестве узла использовать структуру:
```
struct Node 
{
  Node* next;
...
```
Как работает:
- Это однонаправленный список. Т.е. зная первый элемент, можно найти все последующие, но зная последний, мы знаем только его.
- Если элемент последний в списке, то `next = nullptr;`.
- Если элемент не последний то поле next указывает на следующий элемент.
- Поле `value` хранит значение узла.

Что сделать:
- Реализовать метод `bool hasNext();` который возвращает `true` если элемент не последний.
- Реализовать метод `void add(std::string value);` который добавляет в список (после последнего) новый элемент, значение которого равно `value`, этот элемент становится последним.
- Реализовать функцию которая получает на вход ссылку на первый элемент списка и разворачивает его. (Реализовать двумя способами, рекурсивным и не рекурсивным)

5. Все что сделано для односвязного списка сделать и для двусвязного:
```
struct Node 
{
  Node* next;
...
```
Как работает:
- Все правила для односвязного справедливы, добавляется новое правило.
- Указатель prev указывает на предыдущий элемент, у первого элемента в списке он равен `nullptr`

Повторить все упражнения для односвязного списка.

# Урок 6
В ходе развития С++, у тех кто разрабатывал стандарт, стояло 2 приоритета: **быстродействие** и **старый код должен работать**. Вот и изначально когда C++ создавался (сначала он назывался Си с классами) в него перешло все состояние Си. Дальше все это богатство развивалось и росло. Теперь на самом деле C++ нужно рассматривать как 4 языка:
- **С** (Пока только его и изучали)
- **С++** (Думаешь, что уже что-то видел, но на самом деле нет)
- **STL** (Узнаешь сегодня)
- **Шаблоны** (Расскажу о них позже)

Что это значит? Что у нас в разных областях применяются разные правила, и то что хорошо делать в одном месте, плохо делать в другом. До сих пор мы затрагивали только первый раздел. Сегодня рассмотрим STL и некоторые принципы **ООП** которые пришли вместе c С++.

STL - это супер мега крутая библиотека (стандартная), которую ты уже использовал на самом деле, везде где ты писал std::что-то это STL! В этой библиотеке куча всего разного, средства для многопоточной разработки, разные алгоритмы и полезные функции. И что все 100% использую это контейнеры.

Контейнеры STL это некие хранилища данных. На самом деле часто используют просто вместо массивов, потому что так удобно. Остановимся подробно на vector (его жалкое подобие ты реализовывал, тот самый расширяющийся массив), map (это что-то очень крутое, его называют словарем или картой, очень полезная штука) и упомянем array (новый удобный массив, который можно использовать вместо стандартного).

Классы - многие говорят что ничем по сути не отличаются от структур, так и есть. Тут скорее философское отношение, структуры используются для описания набора переменных которые объединены в одну сущность. Классы чаще всего представляют гораздо большую структурную единицу. 

Наследование - один из столпов ООП. Очень важная штука, долгое время будет казаться, что нафиг никому не нужна, но это не так, и когда поймешь это наступит момент "Вот же ш сука!". Это новый для тебя механизм, который позволяет одним классам перенимать поведение и свойства другого класса. Класс у которого переняли свояства называется родительским или базовым, тот который перенял дочерним. На этом пока закончим, сейчас сам всё увидишь.
